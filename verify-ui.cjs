...
(async () => {
  ...
  const baoOptionCount = await page.locator('#communitySelect option[value="宝树台"]').count();
  if (!baoOptionCount) throw new Error('宝树台未出现在小区筛选中');
  await page.locator('#communitySelect').selectOption('宝树台');
  const baoCards = await page.locator('.listing-card').count();
  const baoVisibleNames = await page.locator('.listing-card h3').allTextContents();
  const baoPhotoCards = await page.locator('.listing-card .card-photo img').count();
  if (!baoCards || baoVisibleNames.some(name => name !== '宝树台')) throw new Error('宝树台筛选结果异常');
  await page.screenshot({ path: path.resolve(__dirname, 'preview-detail-ui.png'), fullPage: true });
  console.log(JSON.stringify({ cardCount, photoCount, thumbCount, galleryChanged: firstImage !== secondImage, heroState, persisted, withoutImageCount, rentRangeState, firstCommunity, selectedCommunityCards, selectedCommunityNames, baoOptionCount, baoCards, baoPhotoCards, baoVisibleNames, errors }, null, 2));
  await browser.close();
